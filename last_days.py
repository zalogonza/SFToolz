...
def generate_excel(weekly_data):
    ...
    # Crea un objeto ExcelWriter con modo de 'append'
    writer = pd.ExcelWriter(filename, engine='openpyxl', mode='a', if_sheet_exists='overlay')

    # Nombre único para la nueva hoja basado en la fecha y hora actual
    sheet_name = f"{datetime.today().strftime('%Y-%m-%d at %H%M')}"
    ...
    df = pd.DataFrame(data, columns=[
        'Week Number', 'Area', 'Opportunity Name', 'ForecastCategoryName', 'Amount', 'CloseDate',
        'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday', 'Upside'
    ])

    # Escribe el DataFrame en una nueva hoja
